...
int main(int argc, char** argv) {
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        for (int i = 0; i <= (int)strlen(line)-2; i++ ) {            
            ...
            if ((int)line[i] == 13 ){
                ...
                if (seq1_low < seq2_low) {
                     if ( seq1_high >= seq2_high)  {
                    //seq is ax ay and bx by. If ax <= bx and ax>= bx, then b is contained in a. 
                    total ++;
                   printf("found 1, 1st if! %s \n", line);
                    }
                }
                else if(seq2_low < seq1_low) {
                    if (seq2_high >= seq1_high) {
                        total ++;
                    }
                }
               else if (seq1_low == seq2_low || seq2_high == seq1_high) {
                        total ++;
                        printf("found 1! third if %s \n", line);
                }
    ...
    printf("Total num of lines where one seq is contained in the other: %d \n", total);
    return 0; 
}
